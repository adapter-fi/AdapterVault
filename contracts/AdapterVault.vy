...
def _add_adapter(_adapter: address) -> bool:    
    ...
    #Lock adapter's tokens
    self.adapter_manage(_adapter, 0x0000000000000000000000000000000000000000)
...
def _remove_adapter(_adapter: address, pregen_info: DynArray[Bytes[4096], MAX_ADAPTERS], _rebalance: bool = True, _force: bool = False, _min_assets: uint256 = 0) -> bool:
    ...
    self.adapter_manage(0x0000000000000000000000000000000000000000, _adapter)
...
def _swap_adapters(_adapterOld: address, _adapterNew: address, _force: bool = False, _min_assets : uint256 = 0) -> bool:
    ...
    self.adapter_manage(_adapterNew, _adapterOld)
...
@internal
def adapter_manage(incoming_adapter: address, outgoing_adapter: address):
    ...
    if incoming_adapter != 0x0000000000000000000000000000000000000000:
        ...
        incoming_handled: DynArray[address, 10] = IAdapter(incoming_adapter).managedTokens()
    ...
    if outgoing_adapter != 0x0000000000000000000000000000000000000000:
        outgoing_handled: DynArray[address, 10] = IAdapter(outgoing_adapter).managedTokens()
